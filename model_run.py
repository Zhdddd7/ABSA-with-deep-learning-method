...
from Gated_CNN_model import CNN_Gate_Aspect_Text
from Gated_GCN_model import GCN_Gate_Aspect_Text
import datasets
...
def load_semeval_data(text_field, as_field, sm_field, **kargs):


    semeval_train, semeval_test=data_preparation.split_corpus(data_preparation.create_database('re'))
    ...
    predict_data = datasets.SemEval(text_field, as_field, sm_field, predict_test)

    train_data, dev_data, mixed_data = datasets.SemEval.splits_train_test(text_field, as_field, sm_field,
...
train_iter, test_iter, mixed_test_iter, predict_iter = load_semeval_data(text_field, as_field, sm_field,
...
for t in range(n_trials):
    if args.embed_file == 'w2v':
        ...
        print("Loading GloVe pre-trined embedding...")
    ...
    train = CNN_train
    #if ur using GCN model, then run following
    # model = GCN_Gate_Aspect_Text(args)
    # train =CNN_train
